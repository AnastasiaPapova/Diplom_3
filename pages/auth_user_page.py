...
from locators.burger_locators import MainPageLocators, LoginLocators
...
class AuthUserPage(BasePage):
    ...
    def set_email(self, user_email):
        self.wait_for_element_to_be_clickable(LoginLocators.EMAIL_FIELD)
        self.click_on_element(LoginLocators.EMAIL_FIELD)
        self.set_text(LoginLocators.EMAIL_FIELD, user_email)

    @allure.step('Заполняем поле "Пароль"')
    def set_password(self, user_password):
        self.click_on_element(LoginLocators.PASSWORD_FIELD)
        self.set_text(LoginLocators.PASSWORD_FIELD, user_password)

    @allure.step('Нажимаем кнопку «Войти»')
    def click_login_button(self):
        self.click_on_element(LoginLocators.LOGIN_BUTTON_ANY_FORMS)
    ...
    def check_switch_to_login_page(self):
        self.wait_until_element_visibility(LoginLocators.LOGIN_FORM_HEADER)
